...
def store_true(*args, **kwargs):
    return argument(*args, action="store_true", metavar=NOARG, **kwargs)


def store_false(*args, **kwargs):
    return argument(*args, action="store_false", metavar=NOARG, **kwargs)
    ...
    **kwargs
    ...
    def annotate_group(func):
        ...
        def gen_group(parser):
            g = parser.add_argument_group(_name, description=_description, **kwargs)
            if mutually_exclusive:
...
def command(
    func=None, name=None, description=None, order=None, takes_arguments=False, **kwargs
):
