    ...
    def __cls_argparse__(
        ...
        **exkwargs
        ...
        ap = argparse.ArgumentParser(
            prog=__usage_prog__, description=description, add_help=False
        )
        ap.add_argument(
            "-h",
            "--help",
        ...
        cmdlist = list(
            k for o, k in sorted((v.order, k) for k, v in commands_byname.items())
        )
        ...
        argdo = dict()
        for order, argname, argbunch in sorted(
            [(v.order, k, v) for k, v in arguments.items()]
        ):
        ...
        if print_help:
            spp = ap.add_subparsers(
                dest="command",
                help="(May have subarguments, use -h after them for command specific help)",
            )
        ...
        kwargs["__cls_argparse_cmd_args__"] = (
            tuple(args_sub) if args_sub is not NOARG else None
        )
        obj = cls(**kwargs)
        if no_commands:
            return obj
